...
class Trainer:
    ...
    def build_train_op(self, loss, moving_avg_decay=0.999, max_norm=10.0, dependencies=[]):        
        ...
        summary_op = tf.compat.v1.summary.scalar("global_gradient_norm", tf.linalg.global_norm(gradients))
        ...
        self._model_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES)
        ...
        with tf.compat.v1.variable_scope('backup_variables'):
            self._backup_vars = [tf.compat.v1.get_variable(var.op.name, dtype=var.value().dtype, trainable=False, 
